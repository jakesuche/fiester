import {
  ...
  const LinkSection = ({ title, links }:{title:string,links: typeof developers.links}) => (
    <Stack spacing={2.5}>
      <Title color="text.primary">{title}</Title>
  
      {links.map(({ title }:{title:string}) => (
        <Typography
          key={title}
          variant="body2"
          color="text.primary"
        ...
        <Container>
          <Grid container spacing={8} flexWrap="wrap-reverse">
            ...
            <Grid item xs={12} md={6} lg={5} xl={4}>
              <Stack>
                <Title color="text.primary" sx={{ mb: 1 }}>{subscribe.title}</Title>
